...
def seed_sources(db: Session) -> None:
    """Insert default sources into the database if they are absent."""
    for src in DEFAULT_SOURCES:
        if db.query(Source).filter(Source.id == src["id"]).first():
            continue
        db.add(Source(
            id=src["id"],
            name=src["name"],
...
_HEADERS = {"User-Agent": "NewsAPI/1.0 (RSS aggregator; +https://newsapi.example.com)"}
...
async def refresh_articles(db: Session, source_ids: list[str] | None = None) -> int:
    ...
    db.commit()
    logger.info("refresh_articles: added %d new articles", new_count)
    return new_count
